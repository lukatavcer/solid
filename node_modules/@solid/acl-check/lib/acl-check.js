...
function publisherTrustedApp(kb, doc, aclDoc, modesRequired, origin, docAuths) {
  ...
  let relevant = owners.map(owner => kb.each(owner, ACL('trust'), null, owner.doc()).filter(ta => kb.holds(ta, ACL('trustedApp'), app, owner.doc()))).flat(); // ta's
...
function modesAllowed(kb, doc, directory, aclDoc, agent, origin, trustedOrigins, originTrustedModes = []) {
  ...
  function agentOrGroupOK(auth, agent) {
    ...
    if (kb.each(auth, ACL('agentGroup'), null, aclDoc).some(group => kb.holds(group, VCARD('hasMember'), agent, group.doc()))) {
