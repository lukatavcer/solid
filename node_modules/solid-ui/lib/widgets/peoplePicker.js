...
export class PeoplePicker {
  ...
  render() {
    ...
    if (this.selectedgroup) {
      ...
      this.findAddressBook(this.typeIndex).then(({ book }) => {
        ...
        createNewGroupButton.addEventListener('click', event => {
          this.createNewGroup(book).then(({ group }) => {
            new GroupBuilder(this.element, book, group, this.onSelectGroup).render();
          }).catch(errorBody => {
            this.element.appendChild(errorMessageBlock(document, escape(`Error creating a new group. (${errorBody})`)));
          });
        });
        ...
        this.element.appendChild(errorMessageBlock(document, escape(`Could find your groups. (${err})`)));
      });
    }
  ...
  createNewGroup(book) {
    ...
    return Promise.all(patchPromises).then(() => ({ group })).catch(err => {
...
export class GroupPicker {
  ...
  render() {
    this.loadGroups().then(groups => {
      ...
      this.element.appendChild(errorMessageBlock(document, escape(`There was an error loading your groups. (${err})`)));
    });
    return this;
  }
...
export class GroupBuilder {
  ...
  render() {
    ...
    makeDropTarget(dropContainer, uris => {
      uris.map(uri => {
        this.add(uri).catch(err => {
          this.element.appendChild(errorMessageBlock(document, escape(`Could not add the given WebId. (${err})`)));
        });
      });
    });
    ...
    groupNameInput.addEventListener('change', event => {
      this.setGroupName(event.target.value).catch(err => {
        this.element.appendChild(errorMessageBlock(document, `Error changing group name. (${err})`));
      });
    });
  ...
  handleRemove(webIdNode) {
    return event => {
      const statement = rdf.st(this.group, ns.vcard('hasMember'), webIdNode);
      return patch(this.group.doc().uri, { toDel: [statement] }).then(() => {
        kb.remove(statement);
  ...
  setGroupName(name) {
    ...
    const updatePromises = [this.group.doc(), groupIndex].map(namedGraph => {
      ...
      return patch(namedGraph.value, { toDel: oldNameStatements, toIns: [newNameStatement] }).then(solidResponse => {
