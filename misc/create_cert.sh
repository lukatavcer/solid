...
# DNS accordingly. 
[alt_names]
DNS.1 = luka.example.com
URI = "https://lukatavcer.example.com:8443/profile/card#me"
EOF
...
openssl req \
  ...
  -nodes \
  -x509 \
  -subj "/C=SI/ST=Slovenia/L=state-institutions-luka/O=sigen-ca-luka" \
  -keyout "${DIR}/ca.key" \
  -out "${DIR}/ca.crt"
#
...
# the bits to 4096.
openssl genrsa -out "${DIR}/solid.key" 2048

# Generate a CSR using the configuration and the key just generated. We will
# give this CSR to our CA to sign.
openssl req \
  -new -key "${DIR}/solid.key" \
  -out "${DIR}/solid.csr" \
...
openssl x509 -in "${DIR}/solid.crt" -noout -text

# Convert to .pfx
openssl pkcs12 -export -out "${DIR}/solid.pfx" -inkey "${DIR}/solid.key" -in "${DIR}/solid.crt"

# Get public key
openssl rsa -in "${DIR}/solid.key" -pubout -out "${DIR}/solid_pub.key"

# Display modulus
openssl rsa -pubin -in "${DIR}/solid_pub.key" -text -noout -modulus > "${DIR}/modulus.txt"
